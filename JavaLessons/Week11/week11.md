...
本质：运行时的动态绑定（也就是说后面还有编译期的静态绑定，静态绑定效率更高）
...
为什么：final方法有两个存在的意义：
- 运行加速（选讲：内嵌机制->小方法内联，频繁方法内联，JVM自动优化）
- 禁止重写（选讲：中断继承链）
...
问题：Java方法调用过程中，Jvm是如何知道调用的是哪个类的方法？Jvm又是如何处理？
...
- 一个类可以实现多个接口，但是实现的类必须实现其中规定的所有方法签名（如果这个类是抽象类，那么也允许将方法定义为抽象方法，延迟到子类去实现）
