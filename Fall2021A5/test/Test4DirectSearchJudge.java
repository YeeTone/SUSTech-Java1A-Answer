...
public class Test4DirectSearchJudge {
    ...
    private Object directSearchObject;
    ...
    public void resetObject() throws Throwable {
        ...
        directSearchObject = directSearchClazz.newInstance();
        checkMethodUsing();
        checkValue();
    }
    ...
    private void checkValue() throws Throwable{
        ...
        for (int i = 0; i < values.length; i++) {
            assertEquals(values[i],fields[i].get(directSearchObject));
        }
    }
    ...
    private void randomJudge(int ns,int cases) throws Throwable {
        assertTimeoutPreemptively(Duration.ofMillis(3000),()->{
            Set<String> nodeA2Z = generateNodes(ns);
            String[][] fns = generateFlights(cases, nodeA2Z);
            addFlights(fns);
            String[][] back_propagation = free_fast_reverse(fns);
            addFlights(back_propagation);
            String[] ports = String.join("", nodeA2Z).split("");
            checkBiDirections(fns, ports);
        });

    }

    private void checkDirectSearchRoutes(String[][] testcases, String depart, String arrive) throws Throwable {
        List<String> built = buildDirectSearchRoutes(testcases, depart, arrive);

        List<String> actual_airline = AirlineUtil.toModifiableList((List<String>) searchAllRoutesMethod_Airline.invoke(sustechAirlineObject, depart, arrive, directSearchObject));
        List<String> actual_search = AirlineUtil.toModifiableList((List<String>) searchRoutesMethod_Search.invoke(directSearchObject, depart, arrive));
    ...
    private void checkDirectSearchBestRoute(String[][] testcases, String depart, String arrive, String searchPlanName) throws Throwable {
        String bestRoute = buildBestRoute(testcases, depart, arrive, searchPlanName);
        Object searchPlan = searchPlanMap.get(searchPlanName);
        String actual_airline = (String) searchBestRouteMethod_Airline.invoke(sustechAirlineObject, depart, arrive, directSearchObject, searchPlan);
        String actual_search = (String) searchBestRouteMethod_Search.invoke(directSearchObject, depart, arrive, searchPlan);
    ...
    private String buildBestRoute(String[][] testcases, String depart, String arrive, String searchPlanName) {
        String answer = null;
        int currentLeastValue = 999999999;

        for (String[] fn : testcases) {
            if (!(depart.equals(fn[1]) && arrive.equals(fn[2]))) {
                continue;
            }

            int currentValue;
            if (searchPlanName.equals("LESS_TIME")) {
                currentValue = Integer.parseInt(fn[5]) * 60 + Integer.parseInt(fn[6]) -
                        Integer.parseInt(fn[3]) * 60 - Integer.parseInt(fn[4]);
            } else {
                currentValue = Integer.parseInt(fn[7]);
            }

            if (currentValue < currentLeastValue) {
                answer = String.format("%s [%s -> %s] %02d:%02d -> %02d:%02d (%d)",
                ...
                currentLeastValue = currentValue;
            }
        }
    ...
    private List<String> buildDirectSearchRoutes(String[][] testcases, String depart, String arrive) {
        List<String> result = new ArrayList<>();
        for (String[] fn : testcases) {
            if (fn[1].equals(depart) && fn[2].equals(arrive)) {
    ...
    private String[][] free_fast_reverse(String[][] testcases) {
        String[][] result = new String[testcases.length][];
        for (int i = 0; i < testcases.length; i++) {
            result[i] = testcases[i].clone();
            for (int j = 3; j <= 7; j++) {
                result[i][j] = "0";
            }
    ...
    private void checkBiDirections(String[][] testcases, String[] ports) throws Throwable {
        for (int i = 0; i < ports.length; i++) {
            for (int j = 0; j < i; j++) {
                String depart = String.valueOf(ports[i]);
                String arrive = String.valueOf(ports[j]);

                checkDirectSearchRoutes(testcases, depart, arrive);
                checkDirectSearchRoutes(testcases, arrive, depart);

                checkDirectSearchBestRoute(testcases, depart, arrive, "LESS_TIME");
                checkDirectSearchBestRoute(testcases, arrive, depart, "LESS_TIME");

                checkDirectSearchBestRoute(testcases, depart, arrive, "LESS_PRICE");
                checkDirectSearchBestRoute(testcases, arrive, depart, "LESS_PRICE");
            }
        }
    }
