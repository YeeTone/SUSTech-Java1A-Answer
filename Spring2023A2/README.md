...
4. 复原数组第二阶段：对于目前未知的元素，使用110分的解法进行直接爆破。但是爆破完了以后，或者数组元素本身就已知，我们能否继续进行一些推断，得到更多的元素，减少不必要的`Judge.query()`？

首先理解以下推论的正确性：
- 对于`Query[i]`来说，假定`Query[i]`是0（为1是类似的），那么就说明`(i, i+1, i+2)`这三个位置中，0的个数比1多。(`i+1`和`i+2`有可能越界，需要对数组元素取一下模，从而回到数组的刚开始的下标)
  - 如果i是已知且目标int数组值为1，那么我们一定可以确定i+1和i+2的位置都是0
  - 如果i+1是已知且目标int数组值为1，那么类似可以确定i和i+2的位置都是0
  - 如果i+2是已知且目标int数组值为1，那么类似可以确定i和i+1的位置都是0

那么复原数组第二阶段的做法就是110分的升级版：
- 如果元素未知，使用01进行爆破直接求解
- 爆破结束后，或者元素内容本身就已知，对`Query[i]`及其对应的三个元素的性质进行探索，逐一验证`Query[i]`对应的这三个元素是不是已知且符合上述推论的假设。如果符合，那么就直接推断剩下的两个元素。
...
在这一阶段，我们统共有 $\frac{n}{3}$组，除去中间的两组还剩 $(\frac{n}{3} - 2)$组。每组需要消耗 $2$次`query`，因此总共消耗 $(\frac{n}{3} - 2) × 2 = \frac{2n}{3} - 4$次询问。
